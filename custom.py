...
class sourceListBox(QGroupBox):
    ...
    def load(self,data,colorPicture):
        self.columnsNum = 4
        self.boxHeight = 150
        ...
        for img in data:
            ...
            exec('self.lab%s = clickLabel(self.a)'%i)
            exec('self.b = self.lab%s'%i)
            self.b.setGeometry(int(self.a.height()*0.7),int(self.a.height() * 0.7),int(self.a.width()-self.a.height()*0.7),int(self.a.height()*0.3))
            p = QPixmap()
            p.loadFromData(img[0])
            self.b.setPixmap(p)
            self.b.setScaledContents(True)
            exec('self.lab2%s = clickLabel(self.a)' % i)
            exec('self.c = self.lab2%s' % i)
            self.c.setGeometry(0,5,int(self.a.height()*0.7),self.a.height())
            p = QPixmap()
            p.loadFromData(colorPicture[i][0])
            self.c.setPixmap(p)
